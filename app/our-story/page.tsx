        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            ...
            {milestones.map((milestone, index) => {
              ...
              return (
                <div key={index} className="relative mb-16 md:mb-20 lg:mb-24">
                  <div className={`flex flex-col lg:flex-row items-center ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                    ...
                    <div className={`w-full lg:w-5/12 ${isEven ? 'lg:text-right lg:pr-12' : 'lg:text-left lg:pl-12'}`}>
                      <div className="bg-white rounded-lg sm:rounded-xl lg:rounded-2xl p-3 sm:p-4 md:p-8 shadow-md hover:shadow-lg transition-shadow duration-300 border border-blue-100 will-change-transform hover:will-change-transform hover:-translate-y-1">
                        <div className={`flex items-center space-x-2 sm:space-x-3 mb-2 sm:mb-3 md:mb-4 ${isEven ? 'lg:justify-end' : 'lg:justify-start'} justify-start`}>
                          <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-16 md:h-16 bg-gradient-to-br from-blue-100 to-purple-100 rounded-full flex items-center justify-center shadow-sm">
                            <Icon className="w-4 h-4 sm:w-5 sm:h-5 md:w-8 md:h-8 text-blue-800" />
                          </div>
                          <span className="text-base sm:text-lg md:text-3xl font-bold bg-gradient-to-r from-blue-800 to-purple-800 bg-clip-text text-transparent">{milestone.year}</span>
                        </div>
                        <h3 className="text-base sm:text-lg md:text-2xl font-bold text-blue-800 mb-1 sm:mb-2 md:mb-3">{milestone.title}</h3>
                        <p className="text-gray-600 text-xs sm:text-sm md:text-lg leading-relaxed">{milestone.description}</p>
                      </div>
                    </div>

                    {/* Center Icon */}
                    <div className="hidden lg:flex w-2/12 justify-center">
                      <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-br from-blue-800 to-purple-800 rounded-full flex items-center justify-center shadow-lg z-10 ring-4 ring-white">
